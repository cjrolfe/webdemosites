...
Owner sees Folders, Playlists, Permissions, Friends, Activity, Storage, Upload Tool, Architecture, and Settings. Member sees Folders and Playlists only, with no "Add folder"/Rename/Delete controls within Folders.
...
- **Upload Tool** — setup instructions and reference docs for `apps/media-app-cli`, the bulk-import command-line tool: install steps, first-run sign-in (same email + 6-digit code flow as this app), an example command, and its flags (`--parent`, `--concurrency`, `--dry-run`, `--email`). Static reference content only (`UploadTool.tsx`) — no API calls of its own, since the CLI signs in and uploads the same way this app does. Owner-only.
- **Architecture** — a live infrastructure diagram (system overview, the media API's request/data flow, the passwordless auth sequence, and CI/CD deploy topology), rendered client-side from the same Mermaid source as `infra/docs/architecture-diagram.html` (`Architecture.tsx`, via the `mermaid` package) — the two are kept in sync by hand, not generated from one shared file. Owner-only.
- **Settings** — currently just one toggle: whether the splash intro (below) is shown to new sign-ins. Owner-only (`Settings.tsx`, backed by `settings.ts` — `GET /settings` is readable by any signed-in user since every browser needs to check it, `PATCH /settings` is Owner-only).

## Splash intro

A one-time "studio ident" splash (`Splash.tsx`) plays right after sign-in, before the app shell — a broadsword push-in with a real HTML "Swordthain" wordmark stamped over the crossguard (never baked into the generated video/image, so it stays crisp) and a music sting. Plays after sign-in rather than before Login so its admin kill switch (the Settings tab above) can check a real server-side setting — this API has no unauthenticated routes, so checking before a session exists wasn't an option. Shown once per browser ever (`localStorage`'s `swordthain_intro_seen`, set in `intro.ts`), skipped entirely for `prefers-reduced-motion` visitors, and always skippable (visible Skip control, `Escape`). Requires a real click before the `<video>` even mounts — same trick `Lightbox.tsx` already relies on for its own video, since sound can't autoplay without a prior user gesture.
